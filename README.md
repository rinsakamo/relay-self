...
The first executable `RelayEngine` seam is now a transient finite-choice mechanism rather than a model owner. It receives a bounded request with finite admissible choices and provenance-bearing model-facing context, asks a replaceable provider once in `BOUNDED` mode, and makes at most one explicit `THINK` escalation when the bounded result is cognitively unresolved or outside the admissible choice set and the caller permits escalation. The same transient result records content-free cognitive-work facts: provider-call count, per-attempt host elapsed time, provider-reported token usage/finish reason when available, the provider request's max-output bound, and comparison against an optional caller-supplied soft wall-time allocation. A soft allocation is observational only: exceeding it does not cancel provider execution or change RESOLVED/UNRESOLVED meaning. The result remains transient cognition: it does not authorize Actions, mutate Current Intent or Skill state, establish World truth, or persist model output. A target-local llama.cpp adapter now realizes this provider seam through the local OpenAI-compatible chat-completions endpoint. Malformed model content, invalid envelopes, and non-stop completions fail as provider/protocol errors rather than being relabelled as cognitive uncertainty. The target-local llama.cpp path now also has recorded live qualification evidence: #140 qualified the real localhost RelayEngine/provider path, and the fresh #141 terminal run on the exact qualified main subject exercised the real provider twice while preserving the transient-cognition boundary. These results qualify the bounded exercised path; they are not a claim of general model quality.
