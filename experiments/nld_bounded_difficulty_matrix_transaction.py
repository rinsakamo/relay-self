...
def build_matrix_command(
    *,
    ...
    command = [
        ...
        "-B",
        "-m",
        "experiments.nld_bounded_difficulty_matrix",
    ]
    if run:
        command.append("--run")
    ...
    *,
    ...
    if not isinstance(payload, dict):
        raise PhysicalTransactionError(
            "bounded matrix evidence must be a JSON object"
        )

    if payload.get("evidence_class") != (
        "actual-model bounded difficulty matrix"
    ):
        raise PhysicalTransactionError(
            "bounded matrix payload has the wrong evidence class"
        )

    if payload.get("model_id") != model_id:
        raise PhysicalTransactionError(
            "bounded matrix payload model id does not match"
        )

    if payload.get("dtype") != "bf16":
        raise PhysicalTransactionError(
            "bounded matrix payload did not use canonical bf16"
        )
    ...
    if unavailable:
        raise PhysicalTransactionError(
            "native NLD methods unavailable: "
            + ", ".join(unavailable)
        )
    ...
    if not isinstance(warmup, list) or len(warmup) != len(required_modes):
        raise PhysicalTransactionError(
            "bounded matrix payload must contain one warm-up call per mode"
        )
    ...
    if len(observations) != 54:
        raise PhysicalTransactionError(
            f"expected 54 measured observations; found {len(observations)}"
        )
    ...
    for item in observations:
        ...
        if key not in cells:
            raise PhysicalTransactionError(
                f"unexpected bounded matrix cell: {key!r}"
            )
    ...
    for case_id in expected_cases:
        ...
        compact_cases[case_id] = {
            ...
            "observation_count": case_summary.get(
                "observation_count"
            ),
            "modes": compact_modes,
        }
...
def _initial_summary(
    *,
    ...
    return {
        ...
        "model_id": model_id,
        "qualification_scope": "nld_3b_bounded_difficulty_native_mode_matrix",
        "canonical_runtime": {
...
def run_transaction(
    *,
    ...
    try:
        ...
        dry_command = build_matrix_command(
            model_id=model_id,
        ...
        if dry_returncode != 0:
            raise PhysicalTransactionError(
                f"dry-run matrix probe exited with code {dry_returncode}"
            )
        ...
        if (
            ...
            != "bounded difficulty matrix plan only"
        ):
            raise PhysicalTransactionError(
        ...
        actual_command = build_matrix_command(
            model_id=model_id,
        ...
        if actual_returncode != 0:
            raise PhysicalTransactionError(
                f"actual matrix probe exited with code {actual_returncode}"
            )

        actual_payload = _load_json(actual_output)
        summary["observation"] = validate_matrix_payload(
            actual_payload,
            model_id=model_id,
        )
        _complete_stage(summary, stage)

        summary["status"] = "BOUNDED_MATRIX_PASS"
        summary["non_claims"] = [
            "BOUNDED_MATRIX_PASS is a structural physical-evidence result.",
            "Wrong or invalid bounded outputs remain valid observations.",
            "Case structure is not a validated scalar difficulty scale.",
            "No adaptive RelayEngine mode policy is promoted.",
            "This transaction does not test long-form throughput or thinking depth.",
        ]
        _write_json(summary_path, summary)
        return 0
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Run the NLD-3B bounded difficulty by native-mode "
