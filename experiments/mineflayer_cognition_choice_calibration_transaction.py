...
EXPECTED_MODEL_CALLS = len(calibration.CALIBRATION_CONDITIONS) * CALIBRATION_REPEATS


def physical_requests(model: str) -> dict[str, dict[str, object]]:
    rendered = calibration.render_calibration_requests(model)
...
def planned_request_ledger(model: str) -> list[dict[str, object]]:
    ...
    conditions = tuple(calibration.CALIBRATION_CONDITIONS)
    ...
    if len(rows) != EXPECTED_MODEL_CALLS:
        raise AssertionError("calibration ledger size drift")
    return rows
    ...
    for item in planned:
        ...
        try:
            ...
            raise physical.PhysicalTransactionError(
                "calibration model call failed without retry at "
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Own one llama.cpp lifetime for the #320 "
            "candidate-choice calibration."
        )
    )
    ...
    evidence_root = (
        ...
        if args.evidence_root
            ...
            tempfile.mkdtemp(
                prefix="relay-self-choice-calibration-"
            )
        )
    )
    if args.evidence_root:
    ...
    try:
        ...
        physical._write_json(
            evidence_root / "calibration-result.json",
            result,
        )
