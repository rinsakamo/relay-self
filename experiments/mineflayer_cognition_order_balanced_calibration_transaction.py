...
FORMAT_VERSION = 1
EXPECTED_MODEL_CALLS = balanced.EXPECTED_MODEL_CALLS


def physical_requests(model: str) -> list[dict[str, object]]:
    rendered = balanced.render_planned_requests(model)
    output: list[dict[str, object]] = []
    for item in rendered:
        request_body = copy.deepcopy(item["request"])
        ...
        request_hash = physical._sha256_bytes(
    ...
    if len(output) != EXPECTED_MODEL_CALLS:
        raise AssertionError("physical request count drift")
    return output


def planned_request_ledger(model: str) -> list[dict[str, object]]:
    rows = physical_requests(model)
    if len(rows) != EXPECTED_MODEL_CALLS:
        raise AssertionError("order-balanced ledger size drift")
    return rows
    ...
    for execution_index, item in enumerate(planned):
        ...
        try:
            ...
            raise physical.PhysicalTransactionError(
                "order-balanced calibration model call failed without retry at "
                f"index {execution_index} mapping {item['geometryMapping']} "
                f"permutation {item['permutation']}: "
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Own one llama.cpp lifetime for the #326 "
            "exhaustive candidate-order calibration."
        )
    )
    ...
    evidence_root = (
        ...
        if args.evidence_root
            ...
            tempfile.mkdtemp(
                prefix="relay-self-order-balanced-calibration-"
            )
        )
    )
    if args.evidence_root:
    ...
    summary: dict[str, object] = {
        ...
        "completedStages": [],
        "calibrationOwner": 326,
        "parentCalibrationOwner": 323,
        "scientificOwner": 46,
        "geometryMappingCount": len(balanced.GEOMETRY_MAPPINGS),
        "permutationCount": len(balanced.PERMUTATIONS),
        "calibrationRepeats": balanced.EXPECTED_REPEATS,
    }
    ...
    try:
        ...
        physical._write_json(evidence_root / "binding.json", binding)
        ...
        physical._write_json(
            evidence_root / "order-balanced-calibration-result.json",
            result,
        )
        ...
        physical._write_json(evidence_root / "cleanup.json", cleanup)
